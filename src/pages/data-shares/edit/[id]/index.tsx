...
import { useRoqClient, useDataShareFindFirst } from 'lib/roq';
...
function DataShareEditPage() {
  ...
  const { data, error, isLoading, mutate } = useDataShareFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: DataShareInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.data_share.update({
        data: values as RoqTypes.data_share,
        where: {
          id,
        },
      });
