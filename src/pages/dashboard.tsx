...
import { useRoqPlatformUserProfiles, useDataShareCount, useRoleCount, useUserRoleCount } from 'lib/roq';

import { UserRoleListPage as ShowCaseList } from 'pages/user-roles';

function DashboardPage() {
  const { session } = useSession();

  const dataShareStat = useDataShareCount();

  const roleStat = useRoleCount();

  const userRoleStat = useUserRoleCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Data Shares',
      result: dataShareStat?.data,
      isLoading: dataShareStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`user-roles`} passHref legacyBehavior>
                  <Button
